...
<br >
To build the wordcloud, the TermDocumentMatrix is required, since I have collected all last statments as a single document, inspecting it provides the frequency of the words used by the death-row inmates. Since, I want to use <b>wordcloud2</b> a javascript based package, it is expecting the data as a data.frame with the words and the related frequencies in each column.
...
<li><a href="">nrc</a>: A list of English words and their associations with eight basic emotions (anger, fear, anticipation, trust, surprise, sadness, joy, and disgust) and two sentiments (negative and positive). The annotations were manually done through crowdsourcing. </li>
</ul>
...
```
<br >
The <b>AFINN</b> lexicon, indicates that the death-row inmate's last statements contained words that had more positive sentiments than negative. Infact, the most negative sentiment(a -5 on the AFINN scale) had a single word.
<br >
<br >
...
```
<br >
Just two sentiments in the <b>bing</b> lexicon, but agin confirm, that last statements were more positive than negative. Overall, Love, Thank and Peace appear in the positive sentiment, while Death, Pain and Hate are correctly grouped as negative.
<br >
<br >
